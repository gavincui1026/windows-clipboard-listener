 ...
 */
 ...
 // ✅ Keccak-256 ρ 偏移量（行优先：index = x + 5*y）
 //   r[y][x] =
 //   { { 0, 36,  3, 41, 18 },
 //     { 1, 44, 10, 45,  2 },
 //     {62,  6, 43, 15, 61 },
 ...
 // ✅ 结合 rho+pi（按源格子取 ρ 偏移；与上方表一致的行优先索引）
 __device__ void keccak_rho_pi(uint64_t state[25]) {
     ...
     for (int t = 0; t < 24; t++) {
         int X = y;
         int Y = (2 * x + 3 * y) % 5;  // π 映射
         int dst = X + 5 * Y;          // 目标槽
         int src = x + 5 * y;          // 源槽（用于取 ρ 偏移）
         uint64_t temp = state[dst];
         int r = KECCAK_ROTATION_OFFSETS[src];
         state[dst] = ROTL64(current, r);
         current = temp;
         x = X; y = Y;
     }
 }
 ...
 __device__ void keccak_chi(uint64_t state[25]) {
     uint64_t temp[5];
     for (int y = 0; y < 5; y++) {
         for (int x = 0; x < 5; x++) temp[x] = state[x + 5 * y];
         for (int x = 0; x < 5; x++) {
 ...
 // 完整的Keccak-256实现（Keccak padding：0x01 ... 0x80）
 ...
 __device__ uint32_t sha256_ch(uint32_t x, uint32_t y, uint32_t z) { return (x & y) ^ (~x & z); }
 __device__ uint32_t sha256_maj(uint32_t x, uint32_t y, uint32_t z) { return (x & y) ^ (x & z) ^ (y & z); }
 __device__ uint32_t sha256_sig0(uint32_t x) { return sha256_rotr(x, 2) ^ sha256_rotr(x, 13) ^ sha256_rotr(x, 22); }
 __device__ uint32_t sha256_sig1(uint32_t x) { return sha256_rotr(x, 6) ^ sha256_rotr(x, 11) ^ sha256_rotr(x, 25); }
 __device__ uint32_t sha256_gamma0(uint32_t x) { return sha256_rotr(x, 7) ^ sha256_rotr(x, 18) ^ (x >> 3); }
 __device__ uint32_t sha256_gamma1(uint32_t x) { return sha256_rotr(x, 17) ^ sha256_rotr(x, 19) ^ (x >> 10); }
 ...
 __device__ void sha256(uint8_t* output, const uint8_t* input, size_t len) {
     ...
     if (remaining <= 55) {
         for (int i = 0; i < 8; i++) padded[56 + i] = (uint8_t)((bitLen >> ((7 - i) * 8)) & 0xFFULL);
         uint32_t w[64];
         for (int i = 0; i < 16; i++) {
         ...
         for (int i = 16; i < 64; i++) w[i] = sha256_gamma1(w[i-2]) + w[i-7] + sha256_gamma0(w[i-15]) + w[i-16];
         uint32_t a = h[0], b = h[1], c = h[2], d = h[3];
         ...
         for (int i = 0; i < 8; i++) padded[120 + i] = (uint8_t)((bitLen >> ((7 - i) * 8)) & 0xFFULL);
         // 块1
         {
             uint32_t w[64];
             for (int i = 0; i < 16; i++) {
 ...
 __device__ void base58_encode(char* output, const uint8_t* input, size_t len) {
     ...
     int out_idx = 0;
     int zeros = 0;
     for (int i = 0; i < (int)len && input[i] == 0; i++) zeros++;
     ...
     while (len > 0) {
         ...
         while (len > 0 && temp[0] == 0) {
             for (int i = 0; i < (int)len - 1; i++) temp[i] = temp[i + 1];
             len--;
         }
     }
     for (int i = 0; i < zeros; i++) output[out_idx++] = '1';
 ...
 __device__ void generate_tron_address(const uint256_t* private_key, char* address) {
     ...
     uint8_t hash[32];
     keccak256(hash, pubkey65 + 1, 64); // (X||Y)
 ...
 __device__ void address_from_point(const JPoint* P_jacobian, char* address) {
     ...
     uint8_t hash[32]; keccak256(hash, pubkey65 + 1, 64);
     uint8_t addr_bytes[21]; addr_bytes[0] = 0x41; memcpy(addr_bytes + 1, hash + 12, 20);
     uint8_t sha1[32], sha2[32]; sha256(sha1, addr_bytes, 21); sha256(sha2, sha1, 32);
     uint8_t full_addr[25]; memcpy(full_addr, addr_bytes, 21); memcpy(full_addr + 21, sha2, 4);
     base58_encode(address, full_addr, 25);
 }
 ...
 __global__ void generate_batch(
     ...
     const int ITERS_PER_THREAD = 1024;
     const int BATCH = 16;
     JPoint buf[BATCH];
     ...
     for (int start = 0; start < ITERS_PER_THREAD; start += BATCH) {
         ...
         for (int j = 0; j < BATCH; ++j) {
             ...
             if (final_match) {
                 ...
                 bool same_both = true;
                 #pragma unroll
                 for (int t = 0; t < 35; ++t) { if (address[t] != address2[t]) { same_both = false; break; } }
                 if (!same_both) { continue; }
 ...
 __global__ void test_generation_kernel(char* output) {
     if (threadIdx.x == 0 && blockIdx.x == 0) {
         ...
         for (int i = 0; i < 35; i++) output[i] = test_addr[i];
     }
 }
 ...
 extern "C" {
     ...
     long long generate_addresses_gpu(
         ...
         while (((max_attempts <= 0) || (total_attempts < (long long)max_attempts)) && !found) {
             uint64_t seed = 0;
             for (int i = 0; i < 4; i++) seed = (seed << 16) | (rand() & 0xFFFF);
             ...
             int match_count = 0;
             for (int i = 0; i < BATCH_SIZE; i++) if (h_matches[i]) match_count++;
